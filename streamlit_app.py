...
def apply_bias(msg: str, scores: dict) -> dict:
    ...
    if "tt" in text or "ctt" in text:
        ...
        if (re.search(r"(whp|v1p)", text) and re.search(r"slot(.)?(.)?(.)?", text)):
            scores[""] = scores.get("", 0) + 0.2
        if (
            (re.search(r"t(.)k(.)(.).*cpe.*b(.)ru", text) and re.search(r"t(.)t(.)(.)", text))
            or
            (re.search(r"taas", text) and re.search(r"taas-\d", text))
        ):
    ...
    if (re.search(r"ac(.)?(.)?(.)?(.)?(.)?(.)?|xtvt|ak(.)?(.)?(.)?(.)?(.)?(.)?", text) and re.search(r"c(.)?nc(.)?l((.)?e(.)?)?", text)):
            scores["TT Duplicate Activity"] = scores.get("TT Duplicate Activity", 0) + 0.5
            scores["TT TMF-Physical CPE Unsync"] = scores.get("TT TMF-Physical CPE Unsync", 0) - 0.3
